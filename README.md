...
- `F2` **Trade**: `Tab`/`Shift-Tab` switches between local goods and read-only destination comparisons. `↑`/`↓` or `j`/`k` moves only the active region. `n` changes the reusable quantity; `b`/`s` immediately buys or sells that amount. `Shift-B`/`Shift-S` opens a focused one-transaction order with live cost, capacity, maximum-quantity, and limiting-reason feedback; `m` uses the current maximum, `Enter` confirms, and `Esc` cancels. `t` or `Enter` commits only the displayed travel proposal outside an order.
