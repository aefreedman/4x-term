...
- `F3` **Logistics**: `Tab`/`Shift-Tab` moves between Opportunities, Active Contract, and Storage. The focused panel explains what Energy delivery action `Enter` will perform. Opportunities show the complete physical payload, route burns, delivery, fee, and runway change; active contracts explain automatic progress and cancellation limits; storage transfers open their own exact-amount dialog. Energy logistics never reuse ordinary Trade selection or quantity state.
