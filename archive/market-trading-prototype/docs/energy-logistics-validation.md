...
- Ratatui `TestBackend` verifies the dedicated Logistics activity at 160x45 and 80x30: focused Opportunities, Active Contract, and Storage panels explain the physical workflow and dispatch only typed app requests. Storage transfers use operation-specific exact-amount dialogs; ordinary Trade has no Energy quote row, reusable quantity, or immediate buy/sell path.
...
- Routine crates: 113 game-core tests, 25 game-app tests, 15 game-cli unit/boundary tests, 44 game-tui unit/acceptance tests, and 18 routine game-content tests pass; workspace Clippy denies warnings, formatting/content validation, and `git diff --check` pass. The final workspace rerun is recorded with the feature commit.
...
- The player-impact routine gate uses the fast controlled five-tick fixture and passes both reconciliations. App actor and dedicated Logistics TUI buffer tests provide the non-interactive manual contract/storage trace evidence.
